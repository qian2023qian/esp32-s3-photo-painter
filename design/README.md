...
| 项 | 决定 |
|----|------|
| **风格** | **v3「新粗野主义」**（`v3-brutal.html`） |
| **内容宽度** | **「宽」档 = 1600px**（已设为 v3 默认；右下角切档器可改：窄 1120 / 标准 1400 / 宽 1600 / 铺满） |
| **v2** | **保留不删**，`v2-light-print.html` 作参考，不再改动 |
| **下一步** | 把 v3 移植进 `components/user_app_bsp/mode_src/dashboard.html`（接线真实接口） |

> 移植时记得删掉两处设计稿专用元素：顶部「设计稿 · 假数据」提示条、右下角宽度切档器。
> v3 的桌面侧栏断点是 **1400px**：≥1400 显示左侧栏 + 两栏排版；1024~1400 用顶部导航 + 单栏；
> 状态页的「大图 + 设备」例外，从 1024 起就分两栏（单一栏时大图会高得夸张）。
>
> 移植前请务必保留 `main{width:100%}` 这条 —— 见下方「踩过的坑」，去掉它内容会在宽屏下
> 收缩成 fit-content（几百 px），而且每页宽度还不一样。

| 文件 | 风格 | 说明 |
|------|------|------|
| **`v3-brutal.html`** | **新粗野主义（大胆）· 已选定** | PC / 平板 / 手机三档响应式 |
| `v2-light-print.html` | 纸张 / 印刷（浅色克制）· 保留参考 | 同上，同一套信息架构 |
| `index.html` | 选型落地页 | 两版对比 + 打开按钮（发给朋友挑版本用的） |
...
自检过程中抓到并修掉 4 个**真实 bug**（都是肉眼很难发现、真机会出横向滚动条的那类）：
...
4. **⚠️ `main` 收缩成 fit-content（最坑的一个）**：桌面版式里 `main` 是 `.shell` 网格的子项，
   又写了 `margin:0 auto`。按规范，**grid 子项一旦有 auto 外边距就不会被拉伸**，而是收缩到
   自身内容的 `fit-content` 宽度，再由 auto 外边距居中 —— 结果是"窗口越宽、内容看着越窄"，
   2040px 视口下内容只有 **631px**；而且**每页的 fit-content 不同**，实测出现过 543 / 627 /
   1068 / 1152 一堆互相矛盾的数字，导致排查反复对不上。宽度档位也因此形同虚设。
   **修法：`main{width:100%}`**（填满网格轨道，再由 `max-width` + auto 外边距居中）。
   实测 2040 视口下内容 631 → 1309。**移植时这条不能漏。**
