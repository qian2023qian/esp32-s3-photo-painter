...
# 所有 print 强制 flush，保证重定向到日志文件时也实时输出
...
# 每次处理多少张；None 为不限制
_raw_batch = getattr(cfg, "BATCH_LIMIT", None)
BATCH_LIMIT = None
if _raw_batch not in (None, ""):
    try:
        BATCH_LIMIT = int(_raw_batch)
...
# 优先 PATH，其次项目目录下的 exiftool.exe（避免依赖系统 PATH）
EXIFTOOL_BIN = shutil.which("exiftool") or str(ROOT_DIR / "exiftool.exe")
EXIFTOOL_AVAILABLE = bool(EXIFTOOL_BIN) and os.path.isfile(EXIFTOOL_BIN)
        ...
        try:
            ...
            if response_parser is not None:
                try:
                    ...
                    try:
                        print(f"[WARN] 响应内容(截断): {resp.text[:500]!r}")
                    except Exception:
                        pass
                    last_error = str(e)
...
def call_vlm(image_path: Path) -> dict:
    ...
    def _parse_vlm_response(resp):
        ...
        data = resp.json()
        content = data["choices"][0]["message"]["content"]
        # 模型常把 JSON 包在 ```json ... ``` 代码块里，提取首个 { 到末尾 } 之间的内容
